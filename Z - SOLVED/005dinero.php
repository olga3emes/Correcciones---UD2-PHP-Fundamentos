...
<body>
    ...
    echo "Introduzca por URL: dinero<br><br>";
    $cantidad = intval($_GET["dinero"]); //intval para quitar centimillos
    $billetes = [500, 200, 100, 50, 20, 10, 5, 2, 1]; //¿Qué más da si son monedas o billetes? Luego lo escribo...
    ...
    for ($i = 0; $i < count($billetes); $i++) {
        $cant = intdiv($cantidad, $billetes[$i]); // ¿Qué cantidad me hace falta de cada billete? La función intdiv se encarga.
        if ($billetes[$i] > 2) {
